...
default
{
    ...
    listen(integer chan, string name, key id, string msg)
    {
        ...
        {
            llOwnerSay("got past llgetOwnerKey");
            if(msg == "Teleport From")
            {
                llOwnerSay("got passed teleport from");
            ...
            if(msg != "Teleport From")
            {
                ...
                {
                    llOwnerSay("not listening for children");
                    return;            
                }
            }
        }
...
state sitting
{
    state_entry()
    {
        ...
        llRegionSay(doorChan,"Teleport From Child");
        llOwnerSay("regionsay shouted again");
    ...
    listen(integer chan, string name, key id, string msg)
    {
        llOwnerSay("heard while in seated mode child");
        if(llGetOwnerKey(id) == llGetOwner())
        {
            if(chan == doorChan)
            {
